...
async function initializeApp() {
    ...
    try {
        ...
        showStatus(window.i18n.t('errorLoading'), 'error');
    }
}
...
function saveChats() {
    try {
        ...
        if (chats.length > MAX_CHATS) {
            ...
            const newChats = chats.slice(chatsToRemove);
            window.appState.setState(state => ({
                ...state,
                chats: newChats
            }), 'Trim chats to max limit');
            showStatus(`Удалены старые чаты (сохранено ${MAX_CHATS})`, 'info');
        }
        
        // Автосохранение происходит через state manager
        updateChatsCounter();
    } catch (error) {
        console.error('Ошибка сохранения чатов:', error);
        showStatus(window.i18n.t('errorSavingChats'), 'error');
    }
}
...
function createNewChat() {
    ...
    showStatus(window.i18n.t('statusNewChat'), 'success');
...
function loadChat(chatId) {
    ...
    closeSidebarFunction();
    document.getElementById('messageInput').focus();
    showStatus(window.i18n.t('statusChatLoaded'), 'success');
}
...
function deleteChat(chatId, event) {
    ...
    if (confirm('Вы уверены, что хотите удалить этот чат? Все сообщения будут потеряны.')) {
        ...
        renderChat();
        renderChatsList();
        showStatus(window.i18n.t('statusChatDeleted'), 'success');
    }
}
...
function clearAllChatsHandler() {
    ...
    if (confirm('Вы уверены, что хотите удалить ВСЕ чаты? Это действие нельзя отменить.')) {
        ...
        createNewChat();
        showStatus(window.i18n.t('statusAllCleared'), 'success');
    }
}
...
function getEmptyChatHTML() {
    return `
        <div class="empty-chat">
            ...
            <h2 class="text-2xl font-bold mb-2">${window.i18n.t('emptyChatTitle')}</h2>
            <p class="text-lg mb-4">${window.i18n.t('emptyChatDescription')}</p>
            <div class="text-sm text-gray-600 max-w-md">
                <p class="font-semibold mb-2">${window.i18n.t('emptyChatHelp')}</p>
...
async function sendMessage() {
    ...
    if (!API_CONFIG.key) {
        showStatus(window.i18n.t('errorNoApiKey'), 'error');
        showApiKeyModal();
        return;
    }
    ...
    if (message.length > MAX_MESSAGE_LENGTH) {
        showStatus(window.i18n.t('errorMessageTooLong'), 'error');
        return;
    }
    
    if (!navigator.onLine) {
        showStatus(window.i18n.t('errorNetwork'), 'error');
        return;
    }
    ...
    showTypingIndicator();
    showStatus(window.i18n.t('statusConnecting'), 'info');
    ...
    try {
        ...
        showStatus(window.i18n.t('statusConnected'), 'success');
...
function handleAPIError(error) {
    ...
    let userMessage = window.i18n.t('errorSending');
    ...
    if (error.message.includes('401') || error.message.includes('authentication')) {
        ...
        userMessage = window.i18n.t('errorNetwork');
...
function updateOnlineStatus() {
    if (navigator.onLine) {
        showStatus(window.i18n.t('statusOnline'), 'success');
    } else {
        showStatus(window.i18n.t('statusOffline'), 'error');
    }
}
